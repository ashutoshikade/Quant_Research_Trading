# Plotting
    ...
    plot(mpg ~ wt, data=mtcars) # order is y ~ x
    ...
    pdf(plot(x = log(mtcars$wt), 
             ...
             xlim = c(0,2),
             ylim = c(0, 40),
             yaxs="i", xaxs="i"), file="./plot_file_name_code8.pdf")
...
## PRACTICE QUESTIONS
    ...
    as.integer(as.factor(diamonds$clarity))
    colvec <- c("violet", "cyan", "blue", "green", "yellow", "orange", "red", "darkred")
    colvec[as.factor(diamonds$clarity)]
    diamonds$clarity <- as.factor(diamonds$clarity)
    plot(y=log(diamonds$price), x=log(diamonds$carat),
         pch = 20,
         cex=1,
