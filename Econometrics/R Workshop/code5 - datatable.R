...
## PRACTICE QUESTIONS
    ...
    class(flights)
    dt <- as.data.table(flights).   # or ...
    setDT(dt)   # either this or previous
    
    
    #2. What is the average airtime per destination?
    aggregate(dt$air_time, by=list(dt$dest), mean) # does not consider the NA values
    dt[ , mean(air_time, na.rm=T), by=dest] # NA handled
    ...
    delays <- flights[ , lapply(.SD, mean, na.rm=T), by=.(origin, month, year),
